...
def csv_load(file_name):
    document_loader = CSVLoader(file_name)
    return document_loader.load()
...
def document_load_csv(CHROME_DB, file_name):
    # Xóa dữ liệu cũ trong DB
    for collection in CHROME_DB._client.list_collections():
        ids = collection.get()['ids']
        print('REMOVE %s document(s) from %s collection' % (str(len(ids)), collection.name))
    if len(ids): collection.delete(ids)
    print("🧹 ChromaDB đã được xóa sạch.")

    # Load tài liệu từ file CSV
    documents = csv_load(file_name)
    chunks = split_documents(documents)
    add_to_db(CHROME_DB, chunks)
